var Boss = function(state, atlas, x, y){
    ...
    Boss.prototype.moveTowardsEnemy = function(){
        ...
        var y = this.transform.y + (2+2.5*(this.rage/100));
        ...
        if(!this.special){
            ...
            if(this.relocate){
                ...
                if(Math.floor(Kiwi.Utils.GameMath.radiansToDegrees(this.rotation)) == this.angle){
                    if(Kiwi.Geom.Point.distanceBetween(this.mid, player.mid) > this.height/2-60 && !this.animation.getAnimation("attack").isPlaying){
